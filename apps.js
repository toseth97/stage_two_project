...
app.get("/api/:id", async (req, res) => {
    try {
        const identifier = req.params.id.split(" ");
        console.log(identifier[0]);
        if (identifier.length > 1) {
        } else {
            const person =
                (await Person.findOne({ _id: identifier[0] })) ||
                (await Person.findOne({ firstName: identifier[0] })) ||
                (await Person.findOne({ lastName: identifier[0] }));
            if (!person) {
